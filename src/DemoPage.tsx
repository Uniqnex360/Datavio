...
const PROOF_POINTS = [
  { stat: '75%', label: 'Organic traffic increase', sub: 'Within 30 days for a 5,000-SKU retailer' },
  { stat: '1M+', label: 'SKUs scored and enriched', sub: 'For an industrial B2B distributor' },
  { stat: '4×',  label: 'Faster catalog intelligence', sub: 'Vs. manual audit processes' },
  { stat: '40%', label: 'Fewer product returns', sub: 'Through accurate, complete listings' },
];
...
export default function DemoPage({ navigate }: { navigate: (to: string) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: dbError } = await supabase.from('demo_requests').insert({
      ...
      other_challenge: form.catalog_challenge === 'Other' ? form.other_challenge : '',
    });

    setLoading(false);
    if (dbError) {
      setError('Something went wrong. Please try again or email us at info@datavio.com');
    } else {
      setSubmitted(true);
    }
  };
    ...
    <div className="min-h-screen bg-white font-sans antialiased flex flex-col">
      ...
      <header className="bg-white border-b border-slate-100 py-4 px-6 flex items-center justify-between flex-shrink-0">
        <a href="/" onClick={(e) => { e.preventDefault(); navigate('/'); }} className="flex-shrink-0">
          <img src="/Datavio_logo.png" alt="DatavioAI" className="h-12 w-auto" />
        </a>
        <a
          href="/"
      ...
      <div className="bg-[#1B2F6E] relative overflow-hidden flex-shrink-0">
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-14 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-white leading-[1.1] mb-5">
            See Exactly Where Your Catalog<br />
          ...
          <p className="text-white/70 text-lg max-w-2xl mx-auto leading-relaxed mb-6">
            We'll assess your product data and deliver a full catalog quality report showing where your data stands today and the exact steps to bring more revenue from it.
          </p>
          <div className="inline-flex items-center gap-2 bg-[#00C853]/20 border border-[#00C853]/40 text-[#00C853] font-semibold text-sm px-5 py-2.5 rounded-full">
            <CheckCircle className="w-4 h-4" /> We'll run your first 100 products through the platform — completely free
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-3">
            {submitted ? (
              <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center shadow-sm">
                ...
                <h2 className="text-2xl font-extrabold text-[#1B2F6E] mb-3">You're on the list.</h2>
                <p className="text-slate-500 text-base leading-relaxed max-w-sm mx-auto mb-8">
                  Our catalog intelligence team will be in touch within one business day to confirm your session and start preparing your free catalog quality report.
                </p>
                <div className="bg-slate-50 rounded-xl p-5 text-left border border-slate-100 max-w-sm mx-auto mb-8">
                  <p className="text-xs text-slate-400 uppercase font-bold tracking-wide mb-3">What happens next</p>
                  {[
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
                <h2 className="text-xl font-extrabold text-[#1B2F6E] mb-1">Request Your Free Demo</h2>
                <p className="text-slate-500 text-sm mb-6">Fill in your details and a catalog specialist will reach out within one business day.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input id="first_name" type="text" required placeholder="Jane" value={form.first_name} onChange={set('first_name')} className={inputClass} />
                    </div>
                    <div>
                      <label htmlFor="last_name" className={labelClass}>Last Name <span className="text-red-400">*</span></label>
                      <input id="last_name" type="text" required placeholder="Smith" value={form.last_name} onChange={set('last_name')} className={inputClass} />
                    </div>
                  </div>

                  {/* Email */}
                  <div>
                    <label htmlFor="email" className={labelClass}>Work Email <span className="text-red-400">*</span></label>
                    <input id="email" type="email" required placeholder="jane@company.com" value={form.email} onChange={set('email')} className={inputClass} />
                  </div>

                  {/* Company */}
                  <div>
                    <label htmlFor="company" className={labelClass}>Company Name <span className="text-red-400">*</span></label>
                    <input id="company" type="text" required placeholder="Acme Distribution Co." value={form.company} onChange={set('company')} className={inputClass} />
                  </div>
                  ...
                  {/* Catalog challenge dropdown */}
                  <div>
                    <label htmlFor="catalog_challenge" className={labelClass}>
                      Biggest Catalog Challenge <span className="text-red-400">*</span>
                    </label>
                  ...
                  >
                    {loading ? (
                      ...
                      <span className="flex items-center gap-2">
                        Get My Free Catalog Report <ArrowRight className="w-5 h-5" />
                      </span>
                    )}
                  </button>
                  ...
                  <p className="text-center text-xs text-slate-400 pt-1">
                    ...
                    <a href="/privacy" onClick={(e) => { e.preventDefault(); appNavigate('/privacy'); }} className="underline hover:text-[#00AEEF]">Privacy Policy</a>.
                  </p>
                </form>
              </div>
            )}
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-gradient-to-br from-[#00C853]/10 to-[#00AEEF]/10 border border-[#00C853]/30 rounded-2xl p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-extrabold text-[#1B2F6E] text-sm mb-1">Run 100 products — completely free</p>
                  <p className="text-slate-600 text-xs leading-relaxed">Send us a sample of your product data and we'll run it through the full DatavioAI platform. You'll receive a detailed catalog quality report showing exactly where your data stands and what it'll take to bring more revenue.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
              <p className="text-xs text-slate-400 uppercase font-bold tracking-widest mb-4">What to expect</p>
              <div className="space-y-4">
                {WHAT_TO_EXPECT.map(({ icon: Icon, title, desc }) => (
                  <div key={title} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-bold text-[#1B2F6E] text-sm mb-0.5">{title}</p>
            ...
            <div className="bg-[#1B2F6E] rounded-2xl p-5">
              <p className="text-xs text-white/40 uppercase font-bold tracking-widest mb-4">Proven results</p>
              <div className="grid grid-cols-2 gap-3">
                {PROOF_POINTS.map(({ stat, label, sub }) => (
                  <div key={stat} className="bg-white/10 rounded-xl p-3">
                    <p className="text-xl font-extrabold text-[#00AEEF] mb-0.5">{stat}</p>
                    <p className="text-white text-xs font-bold mb-0.5 leading-tight">{label}</p>
